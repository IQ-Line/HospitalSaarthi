...
type StoreConfigurationPanelProps = {
  /** Hide standalone page chrome when embedded in Onboarding tenant tabs. */
  embedded?: boolean;
};

export function StoreConfigurationPanel({ embedded = false }: StoreConfigurationPanelProps) {
  ...
  return (
    <div className={embedded ? 'flex flex-col gap-4' : 'flex flex-col gap-6 p-6'}>
      {embedded ? (
        <div>
          <h2 className="text-base font-semibold tracking-tight">Store Configuration</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Operational stores for inventory workflows on this facility tenant.
          </p>
        </div>
      ) : (
        <>
