...
import type { Capability } from '../../../../../src/features/user-management/types';
import {
...
} from '../../../../../src/features/user-management/lib/role-capability-md-tree';
...
describe('role-capability-md-tree', () => {
  ...
  it('falls back to the capability_key module segment when provenance is missing', () => {
    ...
    expect(resolveCapabilityCatalogModuleSlug(cap, modules)).toBe('users');
    const enabled = enabledModuleIdsForRoleCapabilities(modules, [cap]);
    expect(enabled.has('l1')).toBe(true);
    expect(enabled.has('l2')).toBe(true);
  });
