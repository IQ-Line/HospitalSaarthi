...
vi.mock('../../../../../src/features/opd-patients/lib/opd-consultation-tenant', () => ({
...
describe('fetchOpdEncounterOverlaysByVisitIds', () => {
  ...
  it('fetches overlays in a single batch request', async () => {
    ...
    expect(apiClient).toHaveBeenCalledWith(
      '/api/v1/opd/prescriptions/by-visits?visit_ids=visit-a%2Cvisit-b',
  ...
  it('returns an empty map when tenant id is unavailable', async () => {
    const { resolveOpdConsultationTenantId } = await import('../../../../../src/features/opd-patients/lib/opd-consultation-tenant');
