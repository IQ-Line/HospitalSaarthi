...
export type HttpConfiguratorTenantModuleEntitlementAdapterOptions = {
  baseUrl: string;
  /** When set, uses internal S2S route (no end-user bearer required). */
  umInternalApiKey?: string;
  timeoutMs?: number;
  maxAttempts?: number;
  cacheTtlMs?: number;
...
export class HttpConfiguratorTenantModuleEntitlementAdapter implements TenantModuleEntitlementPort {
  ...
  private readonly umInternalApiKey?: string;
  ...
  constructor(options: HttpConfiguratorTenantModuleEntitlementAdapterOptions) {
    ...
    this.umInternalApiKey = options.umInternalApiKey?.trim();
  ...
  async listTenantEnabledModuleIds(
    ...
    const useInternalRoute =
      this.umInternalApiKey !== undefined && this.umInternalApiKey.length > 0;
    const url = useInternalRoute
      ? `${this.baseUrl}/api/configurator/v1/internal/tenants/${encodeURIComponent(tenantId)}/enabled-module-ids`
      : `${this.baseUrl}/api/configurator/v1/tenants/${encodeURIComponent(tenantId)}/modules?is_active=true`;
    const headers: Record<string, string> = {
      accept: "application/json",
      "x-tenant-id": tenantId,
    };
    if (useInternalRoute) {
      headers["x-um-internal-key"] = this.umInternalApiKey!;
      ...
      this.log?.(
        {
          ...
          source: "configurator",
          route: useInternalRoute ? "internal" : "user-bearer",
