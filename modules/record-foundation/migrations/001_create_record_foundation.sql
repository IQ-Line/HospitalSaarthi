...
CREATE TABLE IF NOT EXISTS record_foundation.care_contexts (
...
CREATE INDEX IF NOT EXISTS idx_care_contexts_patient_time
    ON record_foundation.care_contexts (iq_tenant_id, patient_id, period_start);
CREATE INDEX IF NOT EXISTS idx_care_contexts_encounter
...
BEGIN
  IF current_setting('citus.coordinator_node_count', true) IS NOT NULL THEN
    PERFORM create_distributed_table('record_foundation.care_contexts', 'iq_tenant_id');
  END IF;
END $$;

-- ─── bundles ─────────────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS record_foundation.bundles (
    id                  UUID        NOT NULL DEFAULT gen_random_uuid(),
    iq_tenant_id        UUID        NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_bundles_care_context
    ON record_foundation.bundles (iq_tenant_id, care_context_id);

CREATE INDEX IF NOT EXISTS idx_bundles_kind
    ON record_foundation.bundles (iq_tenant_id, bundle_kind);

DO $$
BEGIN
  IF current_setting('citus.coordinator_node_count', true) IS NOT NULL THEN
    PERFORM create_distributed_table('record_foundation.bundles', 'iq_tenant_id');
  END IF;
END $$;
