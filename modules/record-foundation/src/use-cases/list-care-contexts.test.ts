...
describe("listCareContexts", () => {
  it("returns paginated care contexts filtered by patient_id", async () => {
    ...
    const result = await listCareContexts(
      { careContextRepo },
      "tenant-1",
      { patient_id: "patient-1", linked: false },
    );
  ...
  it("passes linked filter correctly", async () => {
    const careContextRepo = {
      findAll: vi.fn().mockResolvedValue({ data: [], total: 0 }),
    } as unknown as CareContextRepo;

    await listCareContexts(
      { careContextRepo },
      "tenant-1",
      { patient_id: "patient-1", linked: true },
    );
