...
/**
 * Canonical migration source for the `record-foundation` module.
 ...
 *   drizzle.__drizzle_migrations_record_foundation — module-unique so modules
 *   don't share a watermark).
 */
export default defineConfig({
  schema: "./src/schema/tables.ts",
  out: "./migrations",
  dialect: "postgresql",
  schemaFilter: ["record_foundation"],
  dbCredentials: {
    url: process.env["DATABASE_URL"] ?? "",
  },
  migrations: {
    schema: "drizzle",
    table: "__drizzle_migrations_record_foundation",
  },
});
