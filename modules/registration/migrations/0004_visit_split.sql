...
CREATE TABLE IF NOT EXISTS registration.visit (
  ...
  CONSTRAINT visit_pkey PRIMARY KEY (iq_tenant_id, visit_id),
...
-- Backfill visit rows from legacy registration rows (one visit per registration row).
...
-- Deduplicate registration rows to one per patient (keep newest snapshot).
