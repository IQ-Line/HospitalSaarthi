-- NOT applied by configurator:db-migrate (Citus rejects unscoped UPDATE with global_master subquery).
-- Orphan cleanup runs per-tenant at runtime in listEntitlementEnabledModuleIds().
--
...
-- Clears is_core_override before deactivate (chk_tenant_modules_core_always_active).
