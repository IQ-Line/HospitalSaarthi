...
/**
 * Apply pending `configurator` migrations using the drizzle-kit journal under
 * `migrations/` (numbered SQL + meta/_journal.json), tracked in
 * `drizzle.__drizzle_migrations_configurator`. Each migration runs exactly once.
 *
 ...
 * `src/schema/tables.ts`; regenerate with `nx run configurator:db-generate`.
 */
export async function applyConfiguratorSchemaMigration(
  ...
  await applyMigrations(connectionString, MIGRATIONS_DIR, {
    ...
    migrationsTable: "__drizzle_migrations_configurator",
  });
}
