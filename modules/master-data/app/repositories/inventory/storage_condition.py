"""Database access for ``inventory_storage_conditions``."""
...
class InventoryStorageConditionRepository(ScopedInventoryCatalogRepository):
    def __init__(self, session: Session, scope: CatalogScope) -> None:
        super().__init__(session, scope)

    def _model(self) -> Any:
        return inventory_storage_condition_model(self._scope)

    def _duplicate_message(self) -> str:
        return "A storage condition with this name already exists."
    ...
    def list_rows(
        ...
        *,
        ...
        if search:
            term = f"%{search.strip()}%"
            filters.append(or_(M.name.ilike(term), M.description.ilike(term)))
