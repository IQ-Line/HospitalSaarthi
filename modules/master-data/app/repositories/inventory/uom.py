"""Database access for ``inventory_uoms``."""
...
class InventoryUomRepository(ScopedInventoryCatalogRepository):
    def __init__(self, session: Session, scope: CatalogScope) -> None:
        super().__init__(session, scope)

    def _model(self) -> Any:
        return inventory_uom_model(self._scope)

    def _duplicate_message(self) -> str:
        return "Another active UOM already uses this name or abbreviation."
    ...
    def list_rows(
        ...
        *,
        ...
        if search:
            term = f"%{search.strip()}%"
            filters.append(or_(M.name.ilike(term), M.abbreviation.ilike(term)))
