"""Database access for the Visitpad ``medicines`` catalog (``public`` vs ``tenant_master``)."""
...
class VisitpadMedicineRepository:
    ...
    def _M(self) -> Any:
        return visitpad_medicine_model(self._scope)
    ...
    def list_medicines(
        ...
        *,
        ...
        if schedule is not None:
            filters.append(M.schedule == schedule)
        if search:
            term = f"%{search.strip()}%"
            filters.append(
                or_(
