"""Database access for the Visitpad ``chief_complaints`` catalog (``global_master`` vs ``tenant_master``)."""
...
class VisitpadChiefComplaintRepository:
    ...
    def _M(self) -> Any:
        return visitpad_chief_complaint_model(self._scope)
    ...
    def list_chief_complaints(
        ...
        *,
        ...
        if triage_priority is not None:
            filters.append(M.triage_priority == triage_priority)
        if search:
            term = f"%{search.strip()}%"
            filters.append(
                or_(
