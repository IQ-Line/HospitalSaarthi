"""Database access for the Visitpad ``allergens`` catalog (``master_global`` vs ``master_tenant``)."""
...
from app.catalog.visitpad.table_models import visitpad_allergen_model
...
class VisitpadAllergenRepository:
    ...
    def _M(self) -> Any:
        return visitpad_allergen_model(self._scope)
    ...
    def list_allergens(
        ...
        *,
        ...
        if allergen_type is not None:
            filters.append(M.allergen_type == allergen_type)
        if search:
            term = f"%{search.strip()}%"
            filters.append(
                or_(
