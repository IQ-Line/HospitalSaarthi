"""Database access for the Visitpad ``vaccines`` catalog (``global_master`` vs ``tenant_master``)."""
...
class VisitpadVaccineRepository:
    ...
    def _M(self) -> Any:
        return visitpad_vaccine_model(self._scope)
        ...
        try:
            ...
            if is_unique_violation(exc):
                raise DuplicateVisitpadCatalogKeyError(
                    "Another active vaccine already uses this code.",
                ) from exc
            raise
