"""Database access for the Visitpad ``procedures`` catalog (``global_master`` vs ``tenant_master``)."""
...
class VisitpadProcedureRepository:
    ...
    def _M(self) -> Any:
        return visitpad_procedure_model(self._scope)
    ...
    def list_procedures(
        ...
        *,
        ...
        if billing_category is not None:
            filters.append(M.billing_category == billing_category)
        if search:
            term = f"%{search.strip()}%"
            filters.append(
                or_(
        ...
        page_stmt = (
            select(M, cnt)
            .where(*filters)
            .order_by(M.display_order, M.cpt_code)
            .offset(offset)
            .limit(limit)
        )
    ...
    def list_import_key_strings(self) -> list[str]:
        ...
        if self._scope.is_tenant:
            filters.append(M.iq_tenant_id == self._scope.iq_tenant_id)
        stmt = select(M.cpt_code).where(*filters).order_by(M.cpt_code)
