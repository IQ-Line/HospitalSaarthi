"""Database access for the Visitpad ``chronic_illnesses`` catalog (``global_master`` vs ``tenant_master``)."""
...
class VisitpadChronicIllnessRepository:
    ...
    def _M(self) -> Any:
        return visitpad_chronic_illness_model(self._scope)
    ...
    def list_import_key_strings(self) -> list[str]:
        ...
        if self._scope.is_tenant:
            filters.append(M.iq_tenant_id == self._scope.iq_tenant_id)
        stmt = select(M.icd10_code).where(*filters).order_by(M.icd10_code)
