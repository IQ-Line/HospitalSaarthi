"""Rename ``tenant_id`` → ``iq_tenant_id`` on all ``master_tenant`` catalog tables.

Revision ID: 019_tm_iq_tenant_id_col (≤32 chars for ``alembic_version.version_num``)
Revises: 018_procedure_short_name

**No-op (fresh local DB):** ``011`` renames Visitpad ``master_tenant`` columns; ``012`` creates platform tables with ``iq_tenant_id``.
