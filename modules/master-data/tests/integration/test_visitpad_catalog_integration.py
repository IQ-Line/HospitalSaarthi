"""Smoke tests for Visitpad catalog tables (Rx columns, medicines) on real Postgres/Citus."""
...
def test_visitpad_rx_column_duplicate_returns_409(pg_client: TestClient) -> None:
    body = {
        "section": "frequency",
        "display_name": "Daily",
        "code": "qd",
    ...
    r1 = pg_client.post("/api/v1/master-data/visitpad/rx-columns", json=body)
    assert r1.status_code == 201, r1.text
    r2 = pg_client.post("/api/v1/master-data/visitpad/rx-columns", json=body)
    assert r2.status_code == 409, r2.text


def test_visitpad_medicine_create_and_get(pg_client: TestClient) -> None:
    body = {
        "code": "asp500tb",
    ...
    r = pg_client.post("/api/v1/master-data/visitpad/medicines", json=body)
    assert r.status_code == 201, r.text
    mid = r.json()["data"]["id"]
    g = pg_client.get(f"/api/v1/master-data/visitpad/medicines/{mid}")
    assert g.status_code == 200
    data = g.json()["data"]
    assert data["code"] == "asp500tb"
    assert data["price"] == 12.5

    lst = pg_client.get("/api/v1/master-data/visitpad/medicines")
    assert lst.status_code == 200
    codes = {row["code"]: row for row in lst.json()["data"]}
    assert codes["asp500tb"]["price"] == 12.5

    patch = pg_client.patch(
...
def test_visitpad_chronic_illness_create_and_get(pg_client: TestClient) -> None:
    body = {
        "icd10_code": "dm2_t2",
    ...
    r = pg_client.post("/api/v1/master-data/visitpad/chronic-illnesses", json=body)
    assert r.status_code == 201, r.text
    cid = r.json()["data"]["id"]
    g = pg_client.get(f"/api/v1/master-data/visitpad/chronic-illnesses/{cid}")
    assert g.status_code == 200
    data = g.json()["data"]
...
def test_visitpad_procedure_create_and_get(pg_client: TestClient) -> None:
    body = {
        "cpt_code": "ecg_12",
        "short_name": "93000",
    ...
    r = pg_client.post("/api/v1/master-data/visitpad/procedures", json=body)
    assert r.status_code == 201, r.text
    pid = r.json()["data"]["id"]
    g = pg_client.get(f"/api/v1/master-data/visitpad/procedures/{pid}")
    assert g.status_code == 200
    data = g.json()["data"]
...
def test_visitpad_medicine_bulk_import_from_platform(pg_client: TestClient) -> None:
    body = {
        "code": "imp_med",
...
def test_visitpad_rx_column_bulk_import_from_platform(pg_client: TestClient) -> None:
    body = {
        "section": "frequency",
        "display_name": "Weekly",
        "code": "qw",
    ...
    r = pg_client.post("/api/v1/master-data/visitpad/rx-columns", json=body)
    assert r.status_code == 201, r.text
    rid = r.json()["data"]["id"]
    imp = pg_client.post(
...
def test_visitpad_vital_bulk_import_long_code_from_platform(
    ...
    r = pg_client.post("/api/v1/master-data/visitpad/vitals", json=body)
    assert r.status_code == 201, r.text
    vid = r.json()["data"]["id"]
    imp = pg_client.post(
...
def test_visitpad_import_rejects_inactive_platform_row(pg_client: TestClient) -> None:
    body = {
        "code": "inact_med",
