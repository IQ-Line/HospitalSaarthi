...
- Adapter service running on `:3007` with `ABDM_M3_MOCK_GATEWAY=true` (set `ABDM_M3_LOOPBACK_HIU=true` for end-to-end loop)
...
|---|---|
| `curl: (7) Failed to connect to localhost port 3007` | Adapter service isn't running. `npx nx run abdm-adapter-svc:serve` |
