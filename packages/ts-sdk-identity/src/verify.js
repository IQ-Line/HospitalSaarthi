...
function resolveMaxTokenAgeSeconds(options) {
    const maxTokenAgeSeconds = options.maxTokenAgeSeconds ?? DEFAULT_MAX_TOKEN_AGE_SECONDS;
    if (!Number.isFinite(maxTokenAgeSeconds) ||
        maxTokenAgeSeconds <= 0 ||
        maxTokenAgeSeconds > MAX_ALLOWED_TOKEN_AGE_SECONDS) {
        throw new IdentityVerificationError(`maxTokenAgeSeconds must be within 1-${MAX_ALLOWED_TOKEN_AGE_SECONDS}`);
    }
    return maxTokenAgeSeconds;
}
function resolveClockSkewSeconds(options) {
    const clockSkewSeconds = options.clockSkewSeconds ?? DEFAULT_CLOCK_SKEW_SECONDS;
    if (!Number.isFinite(clockSkewSeconds) ||
        clockSkewSeconds < 0 ||
        clockSkewSeconds > MAX_CLOCK_SKEW_SECONDS) {
        throw new IdentityVerificationError(`clockSkewSeconds must be within 0-${MAX_CLOCK_SKEW_SECONDS}`);
    }
    return clockSkewSeconds;
}
function resolveAllowedAlgorithms(options) {
...
function validateProtectedHeader(token, allowedAlgorithms) {
...
function resolveOrgIdForPrincipal(tenantId, raw) {
    ...
    if (trimmed === tenantId) {
        throw new IdentityVerificationError("iq_tenant_id and org_id must represent distinct semantic scopes when org_id is set");
    }
    return trimmed;
}
...
export async function verifyToken(token, options) {
    ...
    let payload;
    try {
        const verified = await jwtVerify(token, keyFn, verifyOpts);
