...
| **1.5** | `modules/integration-hub/vitest.config.ts:6` (**systemic — found while fixing 1.2/1.3**) | **16 `src/`-colocated `.test.ts` files never ran** — the `test` target's vitest `include` was `test/**` only, silently omitting all colocated unit tests (fhir-bundle-display, secure-otp-compare, gateway-client.http, m0 bridge use-cases, m2 callbacks, m3 use-cases). False coverage: 44 tests that appeared to exist executed nowhere; 1.2/1.3 were among them (my fixes wouldn't have run). | high | ✅ FIXED — added `src/**/*.test.ts` to `include` (keeps `test/integration/**` + `*.sandbox` out). integration-hub now collects 85 files / 239 tests (was 69 / 195), all green. |
