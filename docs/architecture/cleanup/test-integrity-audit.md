...
| 2.3 | `services/web/test/unit/components/capability-gate.test.ts:5-21` | `gateAllowed` re-implements CapabilityGate all>any>single precedence locally via non-hook helpers; real component's `anyOf`/`allOf` aliases (capability-gate.tsx:21-27) never touched. | low | ✅ FIXED 2026-07-08 — local re-implementation deleted; test renders the REAL `CapabilityGate` (@testing-library/react + happy-dom, real permissions store seeded per case). Covers single allow/deny, `anyOf`/`allOf`, alias props, and allOf>anyOf>single precedence. Mutation-verified: flipping the component's precedence fails the suite. |
...
| 1.4 | `modules/integration-hub/test/unit/integrations/abdm/lib/abdm-signature-verifier.test.ts:23` | Security-critical: only the reject path (missing issuer/audience in prod → false) tested; the signature-valid → `true` path never asserted, so an always-false (or always-true off-prod) verifier passes. Possibly covered by integration — confirm. | low | ✅ FIXED 2026-07-08 — in-test RS256 keypair (jose) served from a local `node:http` JWKS server; strict-prod env with BOTH insecure-bypass vars deleted so the accept path can't short-circuit. Valid signed JWT → `true`; SAME token with tampered payload → `false` (accept discriminates on signature). Mutation-verified both ways: always-false and swallow-errors-as-true each fail the suite. |
...
| 4.2 | `modules/integration-hub/test/integration/integrations/abdm/rest-handlers/scan-share-routes.integration.test.ts:47` | DEAD — `skipIf(!DB_URL)` on `SCAN_SHARE_TEST_DB_URL`, never provisioned in CI. Only coverage of scan-share token redemption / used-token 404 / active-queue SQL. | med | ✅ FIXED 2026-07-08 — rewired to the repo-standard scheme (mirrors 4.1): gates on `TEST_DATABASE_URL`, self-provisioning beforeAll (drops+recreates schemas, applies the module's drizzle journal), new `test:integration` target (`hims_test_integration_hub`), `integration_hub` added to CI DB loop. Proven on real Citus: all 4 scan-share tests execute+pass; still skips cleanly DB-less. |
| 4.3 | `modules/integration-hub/.../abdm/use-cases/{m1,m2,m3}/*.sandbox.integration.test.ts` (7) | DOUBLE-DEAD (vitest `exclude` + `RUN_ABDM_SANDBOX_TESTS` + live NHA creds). Acceptable manual-only, but M1/M2 chains + M3 HIU/HIP flows have no automated CI coverage beyond one mock loop — should be DOCUMENTED as manual-gated, not counted. | med | ✅ FIXED 2026-07-08 — documented as manual-gated in `test/integration/integrations/abdm/use-cases/README.md` (per-suite coverage, the real gates verified against config/code, exact run commands, explicit "CI covers only the mocked loop"). Bonus defect found+fixed: `vitest.sandbox.config.ts` `include` still pointed at `src/**` after the suites moved to `test/integration/**`, so `test:sandbox` collected **0 files**; now collects all 7. |
