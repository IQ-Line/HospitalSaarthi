...
2. **RHF generics** — ~28–50 errs / ~10–15 files / risk low. Forms use `useForm<z.infer>` (= z.output) or `useForm<z.input>` while `zodResolver` (v5) returns `Resolver<z.input, ctx, z.output>`; schemas use `.default()/.optional()/.transform()` so input ≠ output and the resolver/Control/SubmitHandler chain mismatches. **Fix:** type `useForm<z.input<typeof schema>>`, submit handlers `SubmitHandler<z.output<...>>`, and shared `FormFields` props `UseFormReturn<z.input, unknown, z.output>`. ⚠️ **The two analysts contradicted each other on whether the shared `Control<T>` components need a 2nd generic** (the systemic agent claimed `medicines.tsx` is already clean — it is NOT; it has 22 errors with `useForm<Input>` and `Control<>` mismatches). **Resolve empirically: prove the exact minimal fix on `medicines.tsx` first**, then propagate. Contingency: add a defaulted `TT extends FieldValues = T` generic (`control: Control<T, unknown, TT>`) only to the component(s) that still mismatch. No `any`/`as`/`@ts-ignore`/`!`. Masking tradeoff: with `z.input`, `watch/getValues/field.value` read as the looser input shape (the code already guards with `!!` / `?? ''`).
