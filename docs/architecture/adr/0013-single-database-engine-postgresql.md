...
- *Good:* Team familiarity. Developers can reuse MongoDB/Mongoose patterns from the production HIMS.
- *Good:* Schema flexibility. New fields can be added without migrations. Useful during rapid prototyping.
- *Good:* Native document model. Clinical forms with variable fields, nested medication arrays, and polymorphic records map naturally.
- *Bad:* No Citus co-location. MongoDB sharding uses a different distribution mechanism. There is no guarantee that all of a tenant's data across different collections lands on the same shard — cross-module queries may require scatter-gather.
- *Bad:* Weak structural integrity enforcement. MongoDB schema validation exists but is opt-in, limited in expressiveness (no cross-document constraints), and not used in the production HIMS. The [rework-vs-rebuild analysis](../analysis/01-rework-vs-rebuild.md) identified data integrity gaps in the production HIMS's MongoDB schemas (unbounded nested arrays, missing validations, no referential integrity between collections).
- *Bad:* Multi-document transactions are expensive. MongoDB supports ACID transactions across documents, but they carry significant performance overhead and have limitations across shards. Healthcare workflows frequently span multiple related records (visit + prescriptions + billing charges) that must be atomic.
- *Bad:* Reporting requires aggregation pipelines or ETL. MongoDB's aggregation framework is powerful but non-standard — reporting tools, BI integrations, and government-mandated dashboards expect SQL.
...
**"MongoDB has faster reads"** — MongoDB's read speed advantage comes from denormalization: embedding related data in a single document means one disk seek per read. PostgreSQL achieves the same result with denormalized projection tables ([database principles §8](../analysis/03-database-principles.md#8-projection-tables-are-first-class-schema-citizens)) or materialized views. The new architecture already uses projection tables for cross-module read paths — the performance characteristic is identical. For indexed lookups on normalized tables, PostgreSQL with proper indexes matches MongoDB's performance.

**"MongoDB has faster writes"** — MongoDB's default write concern (`w:1`) acknowledges the write before journaling to disk. This is faster but unsafe for healthcare data — a server crash between acknowledgment and journal sync loses the record. With `w:1, j:true` (journal-safe writes, which any healthcare system must use), the performance gap narrows substantially. PostgreSQL with `synchronous_commit = on` (the default) provides equivalent durability guarantees at comparable speed.

**"MongoDB is more flexible"** — This is true, and it is a liability in a healthcare system. The production HIMS stores prescriptions as nested JSON arrays inside visit documents. A malformed entry (missing dosage, invalid drug reference, wrong type) is accepted silently. The new architecture uses typed columns with CHECK constraints specifically to prevent this class of bug. Flexibility is available through JSONB columns where the data is genuinely unstructured ([database principles §12](../analysis/03-database-principles.md#12-json-columns-for-truly-unstructured-data-only)).
...
- External sources:
  - PostgreSQL Global Development Group, "JSON Types", https://www.postgresql.org/docs/current/datatype-json.html, accessed 2026-04-30
  ...
  - MongoDB Inc., "Write Concern", https://www.mongodb.com/docs/manual/reference/write-concern/, accessed 2026-04-30
