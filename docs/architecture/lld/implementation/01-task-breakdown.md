...
| B2 | Citus init script | B1 | `infra/db/initdb/01-citus-init.sql` | `CREATE EXTENSION citus`, `pg_stat_statements`, `uuid-ossp` only. Module schemas (`user_management`, `configurator`, `empi`, `master_global`, `master_tenant`) are migration-owned (drizzle / Alembic) — NOT created here. |
