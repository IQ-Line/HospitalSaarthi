...
| M1 later | `GET /api/abdm/v1/m1/profile/phr-card` | `GET /v3/profile/account/phr-card` | Listed in [02-m1-flows.md](./02-m1-flows.md) §4 |
| M1 later | `GET /api/abdm/v1/m1/profile/qr-code` | `GET /v3/profile/account/qrCode` | §4 |
| M1 later | `POST …/m1/profile/mobile/update/otp` etc. | Profile-account mobile update paths | §5 — different from **enrolment-chain** mobile verify above |
...
- **§2 “Enrol via Mobile OTP”** (`abdm.m1.mobile-otp.v1`) describes **standalone** mobile-only enrolment (`enrol/byMobile`). Phase A implemented **mobile verification inside the Aadhaar enrolment chain** (Postman “mobile verify” under the same txn chain), hence different **platform** paths (`…/mobile-verify/…`). Standalone mobile enrol remains **Phase 1 / later**.
...
## Next phase (after Phase A M1 chain is stable)

- Standalone **`abdm.m1.mobile-otp.v1`** (`POST …/m1/enrol/mobile/otp` + verify → `byMobile`) per §02.
- §5 profile mobile/email update routes.
- §6 login, §7 verify-existing.
- Optional: Zod/AJV route schemas on every handler (dev-guide acceptance).
