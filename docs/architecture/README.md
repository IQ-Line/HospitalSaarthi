...
- [Master Data — Schema Design](lld/master-data/01-schema-design.md) — MVP platform catalog (`picklist` + values, modules tree, permissions, role templates); Configurator-facing registry (config schemas, feature flags); every table exposes a URL-safe slug. Post-launch healthcare reference sketch. Includes [ERD](lld/master-data/master-data.erd.json).
