...
Milestone 2 covers **HIP-initiated linking**, **user-initiated linking**, **consent notify**, and **M3 data transfer** (ack, push, notify). Gateway callbacks are served at **`/api/v3/…`** on `integration-hub-svc` (port **3007** by default). Platform staff APIs live under **`/api/abdm/v1/…`**.
...
| Phase 3b — HI data transfer | Done (real Fidelius crypto) | Request → ack → push `dataPushUrl` → `health-information/notify`. Default is `FideliusEncryptor` (Curve25519 ECDH + HKDF-SHA256 + AES-256-GCM); base64 stub is opt-in via `ABDM_FIDELIUS_USE_STUB=true` |
...
| M3 data transfer | Done (real Fidelius crypto) | HI request → ack → push → `health-information/notify`. Real Fidelius by default; base64 stub only when `ABDM_FIDELIUS_USE_STUB=true` |
| Sandbox integration tests | Done | `RUN_ABDM_SANDBOX_TESTS=1 pnpm -F @hims/integration-hub test:sandbox` |

---

## Environment (copy from template)

[`services/integration-hub-svc/.env.example`](../../services/integration-hub-svc/.env.example):
...
```sql
SELECT * FROM integration_hub.abdm_link_tokens WHERE abha_address = 'test.user@sbx';
SELECT session_id, flow_kind, state FROM integration_hub.abdm_sessions ORDER BY created_at DESC LIMIT 5;
```
...
- [ ] `npx nx run integration-hub:test` passes
...
Unit tests: `npx nx run integration-hub:test`. Typecheck with one-shot `npx nx run integration-hub:typecheck` (fine on WSL2 — only long-lived watch mode is the stall risk; see CLAUDE.md).
